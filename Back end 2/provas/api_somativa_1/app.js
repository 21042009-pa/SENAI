...
app.post('/produtos', async (req,res) => {
    try {
        const atributos = (nome, descricao, preco, diponivel, categoria)
        if (atributos == undefined){
            return res.status(400).json({
                sucesso: false,
                mensagem: "Os atributos precisam ser preenchidos"
            })
        }

        const criado = {}
        if (nome !== undefined) criado.nome = nome.trim()
        if (descricao !== undefined) criado.descricao = descricao.trim()
        if (preco !== undefined) {
            if (typeof preco !== "number" || preco <= 0) {
                return res.status(400).json({
                    sucesso: false,
        ...
        if (categoria !== undefined) criado.categoria = categoria
        if (disponivel !== undefined) criado.disponivel = disponivel
